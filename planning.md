...
Pre-Labeling and Annotation Assistance: Instead of an LLM, I will use a custom Python script relying on Regular Expressions and predefined word lists to pre-label the batch of 250 examples. The script looks for an insults list to flag Hostile Trash Talk, and an analysis_terms list (combined with string length and capitalization checks) to flag Game Analysis. I will export this generated data with an empty notes column. I will then review every single row, fixing instances where the strict regex failed (like sarcasm or friendly insults), and logging my corrections in the notes column to maintain full transparency between the script's guess and my human decision.
